...
public class SphereJohan extends CircleJohan {
    float radiusZ;
    int stackCount;
    int sectorCount;
    public SphereJohan(List<ShaderModuleData> shaderModuleDataList, List<Vector3f> vertices, Vector4f color, List<Float> centerPoint, Float radiusX, Float radiusY, Float radiusZ,
    ...
    public void createParaboloid() {
        ...
        float width = 1.6f;
        for(double v = 0; v<= 2*width; v+=Math.PI/60){
            for(double u = -Math.PI; u<= Math.PI; u+=Math.PI/60){
                float x = 0.015f * (float)(v * Math.cos(u));
                float y = 0.015f * (float)(v * Math.sin(u)); // ketebalan
                float z = 0.03f * (float)(v*v);
    ...
    public void createHalfSphere(){
        ...
        float stackStep = (float)Math.PI / (2 * stackCount);
